  ...
  const add_to_cart = async (
    ...
    if (check_vendor()) {
      ...
      if (old_item_details != undefined && total_price > 0) {
        let final_price =
          (await parseFloat(total_price)) - parseFloat(old_item_details.price);
      ...
      <View style={{alignItems: 'center', justifyContent: 'center'}}>
        ...
        <View
          ...
          <View>
            ...
            <Text
              style={{fontSize: 16, color: colors.theme_fg, fontFamily: bold}}>
              {global.currency}
              {item.price}
            </Text>
          </View>
          ...
          <View
            ...
            <UIStepper
              ...
              initialValue={
                props.cart_items[
                  props.cart_items.findIndex(x => x.product_id == item.id)
                ]
                  ? props.cart_items[
                      props.cart_items.findIndex(x => x.product_id == item.id)
                    ].qty
        ...
        <Text
          numberOfLines={2}
          style={{fontSize: 11, color: colors.grey, fontFamily: regular}}>
          {item.description}
        </Text> */}
      </View>
    </View>
    ...
    <SafeAreaView style={styles.container}>
      ...
      {props.cart_count > 0 && (
        <TouchableOpacity
          ...
          <CustomButton
            label={` View Cart (${props.cart_count})`}
            onPress={view_cart}
          />
        </TouchableOpacity>
      )}
