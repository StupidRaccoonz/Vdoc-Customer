...
const AddressList = props => {
  ...
  const renderItem = ({item}) => (
    <TouchableOpacity
      activeOpacity={1}
      onPress={select_address.bind(this, item)}
      style={{
        flexDirection: 'row',
        borderBottomWidth: 1,
      ...
      <View
        ...
        <Text style={{fontFamily: regular, fontSize: 12, color: colors.grey}}>
          {item.address}
        </Text>
      </View>
      <View
        ...
        <TouchableOpacity
          onPress={save_address.bind(this, item.id, item.address_type)}>
          <Icon
            type={Icons.Ionicons}
            name="create-outline"
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={{padding: 10}}>
        ...
        {count == 0 ? (
          <View style={{marginTop: '20%'}}>
            ...
            <Text style={{alignSelf: 'center', fontFamily: bold}}>
              Sorry no data found
            </Text>
          </View>
        ) : (
